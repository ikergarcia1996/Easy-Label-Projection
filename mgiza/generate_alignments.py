import os
from mgiza.utils import count_lines, run_bash_command, concatenate_files
from typing import List
from mgiza.model_utils import align_corpus
import argparse
import shutil
import uuid


def generate_word_alignments_mgiza(
    ...
    mgizapp_dir: str = "mgiza/mgiza/mgizapp",
):

    if tmp_dir is None:
        tmp_dir = os.path.join(output_dir, f"tmp_dir_mgiza_{str(uuid.uuid4().hex)}")
    ...
    print("Mgiza alignments...")
    align_corpus(
        source_file=source_train_path,
        target_file=target_train_path,
        output_dir=tmp_dir,
